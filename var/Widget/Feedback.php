...
class Widget_Feedback extends Widget_Abstract_Comments implements Widget_Interface_Do
{
    ...
    private function comment()
    {
        ...
        if ($parentId = $this->request->filter('int')->get('parent')) {
            if ($this->options->commentsThreaded && ($parent = $this->db->fetchRow($this->db->select()->from('table.comments')
            ->where('coid = ?', $parentId))) && $this->_content->cid == $parent['cid']) {
                $comment['parent'] = $parentId;
                $comment['parentMail'] = $parent['mail'];
            } else {
        ...
        try {
          $msg = array();
          if ($comment['parent'] && $comment['parentMail']) {
            // 评论被回复，通知读者
            $title = $this->db->fetchRow($this->db->select('title')->from('table.contents')->where('cid = ?', $comment['cid']))['title'];
            $link = $this->_content->permalink . '#comment-' . $commentId;
            $msg['mail'] = $comment['parentMail'];
            $msg['content'] = '<html><body>您在文章《' . $title . '》下的评论收到了回复，请点击链接查看详情：<br><a href="' . 
              $link . '">' . $link . '</a></body></html>';
            $msg['subject'] = '【zyj的博客】评论回复通知';
          } else if (!$comment['parent']) {
            // 评论文章，通知笔者
            $msg['mail'] = "yjzha1996@163.com";
            $msg['content'] = '文章收到新的评论，请前往处理';
            $msg['subject'] = '【zyj的博客】文章评论提醒';
          }
          $client = new Typecho_Http_Client_Adapter_Curl();
          $client->setTimeout(1);
          $client->setData(json_encode($msg));
          $client->setHeader("Content-Type", "application/json;charset=utf-8");
          $client->send("http://apigw.zhayujie.com/commentNotice");
          // $client->send("http://127.0.0.1:5000/api/system/test");
        } catch (Exception $e) {
          echo $e->getMessage();
        }
